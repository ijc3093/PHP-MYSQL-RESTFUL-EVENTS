<?php
    ...
    if($_SESSION['userRole'] == 2 || $_SESSION['userRole'] == 1){
        $result = $db->get_Manager_Events($_SESSION['userRole']);
    ...
    if(isset($_POST['edit'])){
        ...
        $_POST['event'] = sanitize_input($_POST["event"]);
        $_POST['id'] = sanitize_input($_POST["id"]);

        $hold = $db->update_Venue($_POST['name'], $_POST['event'], $_POST['id']);
        if($hold == true){
            echo "<script>alert('Venue updated');</script>";
        }else{
            echo "<script>alert('Venue failed to update');</script>";
        }
        //var_dump($_POST);
    }
...
<html lang="en">
  ...
  <body>
    ...
    <nav class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
        <a class="navbar-brand col-md-3 col-lg-2 mr-0 px-3" href="#">Edit Venue</a>
                                ...
                                <?php
                                        ...
                                        echo'<form action="" method="post" role="form">';
                                            ...
                                            echo'<div class="form-group row">';
                                                    ...
                                                    <div class="col-lg-9">
                                                    <input type="text" class="form-control" name="event" value="' . $post["capacity"] . '" required><br>
                                                    </div>';
